...
import import_helper as imh
# import maze_solver as maze_run
if 'simple_maze' in sys.argv:
    obstacles = imh.dynamic_import('maze.simple_maze')
elif 'obstacles' in sys.argv:
        obstacles = imh.dynamic_import('maze.obstacles')
...
def run_maze_solver(edge):
    '''
    The function solves the maze with hardcoded moves/instructions to reach all the edges
    '''

    if edge == 'right':
        moves = ['forward 19','right', 'forward 79','left', 'forward 24','right','forward 21']
        return moves
    if edge == 'top':
        moves = ['forward 50','left', 'forward 70', 'right', 'forward 149']
        return moves
    if edge == 'left':
        moves = ['back 110', 'left', 'forward 99']
        return moves
    if edge == 'bottom':
        moves = ['back 110', 'left', 'forward 60', 'left', 'forward 40', 'left', 'forward 120', 'right', 'forward 49']
        return moves


def do_mazerun(robot_name, edge):
    '''
    The function calls all the existing commands moves (e.g. top, bottom, left, right) to solve the maze.
    '''
    if edge == '':
        edge = 'top'
    ...
    world.reset_turtle()
    moves = run_maze_solver(edge)

    print(world.position_x)
...
def robot_start():
    ...
    global position_x, position_y, current_direction_index, history,obs
    ...
    world.obstacles.obstacles_list = obstacles.obstacles_list
    world.draw_obstacles(world.obstacles.obstacles_list)
    ...
    if len(obstacles.obstacles_list)>0:
    ...
    world.position_x,world.position_y,world.current_direction_index = 0,0,0
