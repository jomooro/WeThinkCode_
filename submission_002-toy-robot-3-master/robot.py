...
valid_commands = ['off', 'help', 'forward', 'back', 'right', 'left', 'sprint', 'history', 'replay', 'silent', 'replay silent', 'reversed', 'replay reversed', 'replay reversed silent']
...
def history(command):
    ...
    if command.split()[0] in directions:
...
def valid_command(command):
    ...
    or (len(arg1.split()) == 1 and arg1.lower() in valid_commands)\
    or command.lower() in valid_commands or (len(arg1.split()) == 1 and is_int(arg1) and command_name.lower() in valid_commands)\
...
HISTORY - keeps a history of all commands given to it
REPLAY - replays the movement commands
REPLAY SILENT - replays the commands, by showing the resulting updated position without showing the output of each command,
REPLAY REVERSED  - plays back the commands, with full output, in reverse order to how it was invoked originally
REPLAY REVERSED SILENT - plays back the commands in reverse, and only output the number of commands and final position as per the silent flag
"""
...
def do_replay(robot_name, command):
    ...
    for i in range(len(history_list)):
        command = history_list[i]
        replay_command(robot_name, command)
...
def do_replay_silent(robot_name,command):
    ...
    for i in range(len(history_list)):
        command = history_list[i]
        silent_command(robot_name, command)

    return True, f" > {robot_name} replayed {len(history_list)} commands silently."


def silent_command(robot_name,command):
...
def do_replay_reversed(robot_name,command):
    ...
    for i in range(len(history_list)):
        command = history_list[i]
        reverse_command(robot_name, command)

    return True, f" > {robot_name} replayed {len(history_list)} commands in reverse."


def reverse_command(robot_name,command):
...
def do_replay_reversed_silent(robot_name,command):
    ...
    for i in range(len(history_list)):
            command = history_list[i]
            reverse_silent_command(robot_name, command)
    return True, f" > {robot_name} replayed {len(history_list)} commands in reverse silently."


def reverse_silent_command(robot_name,command):
